 ...
 */
function main (root, type, workname) {
  const relation = cssControl(root, type, workname);
  // res -> diff
  // relation -> 依赖关系
  autoGit().then(res => {
    ...
    console.log(JSON.stringify(aRcom));
    writeFile('./examples/dist/export.json', JSON.stringify(aRcom)).then(res => {
